// third iteration
// prints all previous guesses instead of used letters
...
char * checkGuess(const char * guess, const char * answer, int letters[]){
...
void printLetter(char code, char guess[], int i){
    if(code == '-'){cout << WHITE << guess[i];}
    else if(code == '&'){cout << YELLOW << guess[i];}
...
int main(){
    ...
    while(!isCorrect && guesses < 6){
        ...
        if(len < 5){cout << "Not enough letters\n"; repeat = true;}
            ...
            code = checkGuess(guess, answer, letters);
            for(int i = 0; i < 6; i++){
                codeList[guesses][i] = code[i];
                guessList[guesses][i] = guess[i];
            }
            for(int j = 0; j < guesses+1; j++){
                for(int i = 0; i < 5; i++){
                    printLetter(codeList[j][i], guessList[j], i);
                }
                if(j != guesses){
                    cout << endl;
                }
